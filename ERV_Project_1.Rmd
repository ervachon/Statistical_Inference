...
For reproducability, we fixe the seed :  
```{r}
set.seed(19)  
```  
...
Now let's use the formula of the CLT : $\frac{\sqrt n (\bar X_n - \mu)}{\sigma}$ and use n=10, n=50 and n=100 :  
```{r,results='hide', message=FALSE, warning=FALSE}
set.seed(17)
cfunc <- function(n) 
{  
...
## Conclusion : 
- The theorical mean and variancee are quite near the simulmation mean and variance
- with the formula of the CLT we can see a cuvre and with the normal Quantile-Quantile plot the blue circles follow the red line more or less at the extrems but follow it on the middle
